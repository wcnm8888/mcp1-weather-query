...
> 状态：`approved`。F-001、F-002 已完成并关闭；用户已选择 D-001，并仅允许按 Step 门禁推进。本文件不授权自动合并、跨 Step 或执行任何外部发布。
...
| D-001 | 发布候选与发布前审查 | **Step 7 Git/PR 交付已获准** | 用户获得可审查、不会误发布的发布材料 | README 安装示例、CHANGELOG、发布清单、`server.json` 草案/校验、命名检查、安全审查 | 外部上传和注册 | QA 候选已完成双干净安装和 installed-package stdio 复验；用户 UAT 已通过 | S |
...
D-001 是唯一活动任务，Step 6 独立 QA 与用户 UAT 已完成，当前只允许完成 Step 7 Git/PR 交付。任务详情见 `docs/project-management/current-task.md`。不得自动合并、登录/发布或进入 R-001/R-002。
