...
> 状态：`approved`。F-001、F-002、D-001 已完成并关闭；R-001 Draft PR #7 已创建，
> Step 5 等待最新 PR CI。本文件不授权自动进入下一 Step、执行外部发布或进入 R-002。
...
| R-001 | PyPI 外部发布 | **已批准；Draft PR #7 等待 CI** | 外部用户可通过 Python 包索引安装 Server | Trusted Publishing workflow、最终候选、分段授权发布、公开制品与安装验证 | TestPyPI、npm、Registry、GitHub Release、远程托管 | D-001；PyPI Pending Trusted Publisher；用户分段明确授权 | L |
...
当前活动任务为 R-001；Draft PR #7 已创建并等待最新 CI。状态入口见
`docs/project-management/current-task.md`。本 Step 只允许 Draft PR 和非发布 CI；不得自动
进入 Step 6、登录 PyPI、配置 Pending Publisher、上传、创建 tag/Release 或进入 R-002。
