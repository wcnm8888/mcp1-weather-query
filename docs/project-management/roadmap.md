...
> 状态：`approved`。F-001、F-002、D-001 已完成并关闭；R-001 Step 7 已获授权，发布
> 元数据修复 Draft PR #9 CI 已通过并等待用户合并。本文件不授权绕过门禁或进入 R-002。
...
| R-001 | PyPI 外部发布 | **Step 7 修复 PR #9 待合并** | 外部用户可通过 Python 包索引安装 Server | Trusted Publishing workflow、最终候选、分段授权发布、公开制品与安装验证 | TestPyPI、npm、Registry、GitHub Release、远程托管 | Pending Publisher 已配置；Step 7 已授权；PR #9 合并后才可 tag | L |
