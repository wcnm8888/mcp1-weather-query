...
> 状态：`approved`。F-001 已完成并关闭；F-002 Draft PR #3 已打开，等待用户审查。本文件不授权自动合并、跨 Step 或执行任何外部发布。
...
| F-002 | 可安装与可构建闭环 | **Step 6 已完成；Draft PR #3 等待审查** | 用户能从 wheel/sdist 在干净环境安装并启动同一 Server | console entry point、包元数据、License/署名、`uv build`、artifact 审查、干净安装 smoke | 真实上传、Registry 登记 | QA/UAT 已通过；功能分支已推送，Draft PR #3 未合并 | S |
...
F-001 已完成并关闭。F-002 是当前唯一活动任务，Step 6 已完成且 Draft PR #3 为 open/draft；等待用户审查，合并后才可进入 Step 7 收口。
