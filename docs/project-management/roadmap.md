...
| --- | --- | --- | --- | --- | --- | --- | --- |
| F-001 | 一个 Tool 的本地天气闭环 | **进行中；Step 0 已完成** | 用户能在 Inspector/Host 通过 stdio 查询当前天气并获得结构化结果 | Python 工程基线、官方 SDK v2、Open-Meteo 适配器、一个 Tool、错误映射、单元/协议/stdio 测试、Inspector 人工验收 | 打包发布、第二 Tool、HTTP、Agent | 任务卡已批准；等待用户允许进入 Step 1 | M 内的首个垂直切片 |
