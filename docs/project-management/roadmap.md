...
> 状态：`approved`。F-001、F-002、D-001 已完成并关闭；R-001 Step 5 已完成，Draft PR #7
> 等待用户审查/合并。本文件不授权自动进入下一 Step、执行外部发布或进入 R-002。
...
| R-001 | PyPI 外部发布 | **已批准；Step 5 完成，Draft PR #7 待处理** | 外部用户可通过 Python 包索引安装 Server | Trusted Publishing workflow、最终候选、分段授权发布、公开制品与安装验证 | TestPyPI、npm、Registry、GitHub Release、远程托管 | D-001；PyPI Pending Trusted Publisher；用户分段明确授权 | L |
...
当前活动任务为 R-001；Step 5 已完成，Draft PR #7 等待用户审查/合并。状态入口见
`docs/project-management/current-task.md`。不得自动合并、进入 Step 6、登录 PyPI、配置
Pending Publisher、上传、创建 tag/Release 或进入 R-002。
