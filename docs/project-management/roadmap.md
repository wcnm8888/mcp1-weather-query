...
| D-001 | 发布候选与发布前审查 | **Step 7 已交付 Draft PR #5；待用户审查** | 用户获得可审查、不会误发布的发布材料 | README 安装示例、CHANGELOG、发布清单、`server.json` 草案/校验、命名检查、安全审查 | 外部上传和注册 | QA 候选与用户 UAT 已通过；Draft PR #5 为 OPEN | S |
...
D-001 是唯一活动任务，Step 7 已交付 Draft PR #5，等待用户审查并决定 Ready/合并。任务详情见 `docs/project-management/current-task.md`。不得自动合并、登录/发布或进入 R-001/R-002。
