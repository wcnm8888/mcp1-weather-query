# 已归档任务卡：F-002 可安装与可构建闭环

## 任务状态

- 状态：`completed / merged / post_merge_closure_pr_open`
- 项目整体等级：M
- 当前任务等级：S
- 用户选择：已选择 F-002
- 任务卡批准：已批准（2026-08-11）
- 功能 PR：[PR #3](https://github.com/wcnm8888/mcp1-weather-query/pull/3) 已合并
- 合并提交：`ac39554a87b8f8fde85ec6d893484c42d1c6b253`
- 收口 PR：[Draft PR #4](https://github.com/wcnm8888/mcp1-weather-query/pull/4) 等待用户审查
- 环境：local build / clean install / synthetic offline
- 外部发布：不在本任务范围，未授权

本文件是 F-002 的完整归档任务卡。F-001 已归档到
...
- 用户审查并决定是否合并收口 Draft PR #4；
- PR #4 合并后的本地 `main` 最终同步，以及另行确认后的分支清理。
...
- 状态：`completed / merged`（2026-08-11）。
- [x] 精确审查并按意图拆分提交：
...
- [x] 用户审查并合并 PR #3；合并提交为 `ac39554a87b8f8fde85ec6d893484c42d1c6b253`。

### Step 7：合并后收口

- 状态：`closure_pr_open / awaiting_user_review`（2026-08-11）。
- [x] 安全同步本地 `main` 与 `origin/main` 到 PR #3 合并提交 `ac39554`。
- [x] 合并后默认离线门禁通过：`62 passed, 1 skipped`。
- [x] 修复 Windows CRLF/LF 导致的制品文本比较误报；wheel RECORD 严格校验未放宽。
- [x] 归档 F-002，重置 current-task/implementation-plan，更新 roadmap/progress/evidence。
- [x] 创建收口 Draft PR #4；未直接修改 `main`，未进入 D-001。
- [ ] 用户审查并决定是否合并 PR #4；本 Step 不自动执行。
- [ ] PR #4 合并后再次同步本地 `main`；分支删除需用户另行明确确认。
...
- [x] 默认离线门禁和独立 QA 完成。
- [x] 用户 UAT 完成。
- [x] diff、提交、PR 证据已齐全；功能 PR #3 已合并。
- [x] F-002 已归档，current-task 重置，文档与 Git 事实一致。
- [x] 未进入 D-001、PyPI、Registry 或其他发布。
- [x] 没有无关修改、敏感信息或已提交构建/临时制品。

## 审批记录与下一门禁

用户于 2026-08-11 批准本任务卡的目标、范围、非目标、命名、版本、MIT License、构建方案、测试矩阵、Step 地图和完成定义，并确认 14 项推荐决策。PR #3 已由用户合并；Step 7 已完成合并后复验、最小兼容性修复、任务归档和状态文档收口，并创建 Draft PR #4。当前门禁是用户审查 PR #4；不得自动合并、删除分支、选择 D-001 或执行外部发布。
