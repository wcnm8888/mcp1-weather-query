...
- 任务编号：`R-001`
- 状态：`closed / published / verified`
- 风险等级：`L`（公开发布、供应链配置和不可覆盖版本）
- 起点提交：`0d5d7be9271b71143cdbcdf768bfbde5ed4393d0`
- 收口分支：`agent/r-001-step9-release-closure`
- PR 目标：`main`
- 外部发布：`mcp-weather-query==0.1.0` 已通过 Trusted Publishing 发布并完成公开验证
...
- Step 7 的发布元数据阻断已通过 PR #9 修复并合并；精确 `v0.1.0` 已触发生产发布。
- Step 8 已完成公开 PyPI 文件、哈希、attestation、双干净安装和 stdio 验证。
- Step 9 已完成发布后文档整理并形成独立 Git 收口 PR；R-002 未启动。
...
- [x] Step 7：最终发布门禁与精确 `v0.1.0` tag；Trusted Publishing 成功。
- [x] Step 8：公开 PyPI 文件、attestation、安装和 stdio 验证。
- [x] Step 9：发布后文档、Git 收口；停止在 R-002 之前。
...
Step 7 首次最终门禁因公开元数据仍写“未发布”而停止；用户随后授权修复 PR。PR #9 已合并，
本地 `main` 同步到 `bb24624dcc8eb2efce5f4c49c80c850542c4d2b3`。最终门禁通过后创建并
推送 `v0.1.0`，run `31567283749` 的 build 与 production PyPI publish job 均成功。

Step 8 复核生产 PyPI 只包含批准的 wheel 与 sdist；两者的 SHA-256、Metadata 2.4、公开
README、License、依赖与 publish attestations 均符合契约。`pypi-attestations 0.0.30`
对两个制品返回 `OK`。两个项目外 Python 3.12.10 环境分别安装公开 wheel/sdist，均通过
唯一 Tool、现代/Legacy 协议、structuredContent、stdout/stderr、退出码和无残留进程验证。

R-001 已满足完成定义并关闭。MCP Registry、GitHub Release、TestPyPI、新版本和第二 Tool
均未进入范围；下一任务必须由用户从 roadmap 另行选择和批准。

## 历史任务卡

- [`F-001 一个 Tool 的本地天气闭环`](F-001-一个-Tool-的本地天气闭环.md)
- [`F-002 可安装与可构建闭环`](F-002-可安装与可构建闭环.md)
- [`D-001 发布候选与发布前审查`](D-001-发布候选与发布前审查.md)
