...
- 生命周期：roadmap 已批准；F-001 已关闭，F-002 已由用户选择并批准。
- 项目整体等级：M；当前用户价值切片：F-002（S 级）。
- 当前状态：F-002 Step 6 Git/PR 交付已完成，分支为 `feat/f-002-installable-package`，Draft PR #3 已打开，等待用户审查。
...
- 当前只能执行 F-002 已由用户明确允许的 Step；不得跨 Step 或自动进入后续任务。
- F-002 Draft PR #3 已打开；不得自动标记 Ready、合并、删除分支、进入 Step 7 或扩大范围。
