...
- 生命周期：roadmap 已批准；F-001 已关闭，F-002 功能 PR #3 已合并并进入最终收口。
- 项目整体等级：M；当前没有活动用户价值切片。
- 当前状态：F-002 已完成并归档；Draft PR #4 承载合并后复验修复和状态文档收口，等待用户审查。
...
- 当前只允许完成 F-002 Step 7 收口；不得自动选择或进入后续任务。
- Draft PR #4 已打开；不得自动标记 Ready、合并或删除分支。PR 合并和分支删除分别由用户决定。
